...
const gameModes = [
  ...
  },
  {
    ...
    fullDescription:
      "실력을 겨루고 전국 랭킹에 도전하는 경쟁 모드입니다. 최고의 성적을 목표로 하세요.",
  },
];
...
const isGameStartReady = computed(() => {
  ...
  if (selectedGameMode.value?.id === "normal")
...
const startGame = () => {
  ...
  if(selectedGameMode.value.id == "practice") {
    console.log("Practice Game Starting", {
      region: selectedRegion.value,
    });
    startPracticeGame();
  } else if(selectedGameMode.value.id == "normal") {
    console.log("Normal Game Starting", {
      level: selectedLevel.value,
    });
  } else if(selectedGameMode.value.id == "rank") {
    console.log("Rank Game Starting");
  }
...
const startPracticeGame = () => {
  // Implement practice game start logic
  console.log("Practice Game Starting", {
    region: selectedRegion.value,
  });
  this.$axios.post("/api/practice-game", {
    region: selectedRegion.value,
  });
};
