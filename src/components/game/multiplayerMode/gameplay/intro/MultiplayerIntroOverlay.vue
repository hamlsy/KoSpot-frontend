    ...
    <div v-if="showIntro" class="intro-overlay">
      <div class="intro-content">
        <transition name="slide-fade" mode="out-in">
          ...
          <h2 v-else-if="step === 4" key="step4">{{ countdown }}</h2>
        </transition>
      </div>
    </div>
  </transition>
...
export default {
  ...
  methods: {
    ...
    startSyncedIntroSequence() {
      ...
      if (elapsedSinceStart >= 4500) { // 카운트다운 단계
        this.step = 4;
      ...
      } else if (elapsedSinceStart >= 3000) { // 라운드 번호 단계
        this.step = 3;
        this.scheduleNextStep(4500 - elapsedSinceStart);
      } else if (elapsedSinceStart >= 1500) { // "준비되셨나요?" 단계
        this.step = 2;
