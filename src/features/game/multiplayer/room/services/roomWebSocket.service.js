...
class RoomWebSocketService {
  ...
  async connectToRoom(roomId, currentUserId, eventHandlers = {}) {
    try {
      ...
      // WebSocket 연결 (미연결 시만)
      // 자동 재연결은 core.js STOMP reconnectDelay(5초)가 담당합니다.
      if (!this.webSocketManager.isConnected.value) {
        await new Promise((resolve, reject) => {
          this.webSocketManager.connect('/ws', () => resolve());
          // 10초 타임아웃만 설정 (열릴 때만)
          setTimeout(() => reject(new Error('WebSocket 연결 타임아웃')), 10000);
        });
      }

      // 이벤트 핵들러 저장 (재연결 시 재사용)
      ...
      // 자동 재연결은 STOMP reconnectDelay가 담당
      // 여기서 수동 재연결을 시도하지 않습니다.
      return false;
    }
  }
