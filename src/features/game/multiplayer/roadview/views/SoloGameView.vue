...
export default {
    ...
    localPlayers() {
      ...
      return this.gamePlayers.map((player) => {
        ...
        return {
          ...
          ), // memberId가 있으면 사용, 없으면 playerId 사용
  ...
  methods: {
    ...
    setupUICallbacks() {
      ...
      this.soloGameFlow.setCallbacks({
        ...
        onNextRoundShow: () => {
          ...
          const totalPlayers = this.isDummyRuntime
            ? this.gameStore.state.players.length
            : this.gamePlayers.length;

          // NextRoundOverlay 표시
          if (this.$refs.baseGame) {
            this.$refs.baseGame.startNextRound(
        ...
        onGameFinish: (finalGameResult) => {
          ...
          if (!finalGameResult || typeof finalGameResult !== "object") {
            console.warn(
              "[Solo Game] 게임 종료 메시지에 finalGameResult 데이터가 없음",
            );
            return;
          }
          ...
          this.gamePlayers = gamePlayers.map((player) => ({
            ...
            memberId:
              player.memberId != null
                ...
                : player.playerId, // memberId가 없으면 playerId 사용
            nickname: player.nickname || "알 수 없음",
            markerImageUrl: player.markerImageUrl || null,
            totalScore:
              player.totalScore != null ? Number(player.totalScore) : 0,
            roundRank: player.roundRank != null ? Number(player.roundRank) : 0,
            distanceToTarget: player.distanceToTarget || null,
            lastRoundScore: player.lastRoundScore || 0,
          ...
          if (memberId) {
            ...
            const currentPlayerInfo = gamePlayers.find((p) => {
              const playerMemberId =
                p.memberId != null ? String(p.memberId) : null;
              const playerPlayerId =
                p.playerId != null ? String(p.playerId) : null;
              return (
            ...
            if (currentPlayerInfo) {
              if (currentPlayerInfo.markerImageUrl) {
                this.gameStore.state.currentUser.markerImageUrl =
                  currentPlayerInfo.markerImageUrl;
                this.gameStore.state.currentUser.equippedMarker =
                  currentPlayerInfo.markerImageUrl;
              }
        ...
        onRoundResultUpdate: (roundResultData) => {
          ...
          if (
            roundResultData.playerTotalResults &&
            ...
            roundResultData.playerTotalResults.forEach((result, index) => {
              ...
              const player = this.gamePlayers.find((p) => {
                const resultPlayerId =
                  result.playerId != null ? String(result.playerId) : null;
                const resultMemberId =
                  result.memberId != null ? String(result.memberId) : null;
                const pPlayerId =
                  p.playerId != null ? String(p.playerId) : null;
                const pMemberId =
                  p.memberId != null ? String(p.memberId) : null;
                ...
                return (
                  (resultPlayerId &&
                    pPlayerId &&
                    resultPlayerId === pPlayerId) ||
                  (resultMemberId &&
                    pMemberId &&
                    resultMemberId === pMemberId) ||
                  (resultPlayerId &&
                    pMemberId &&
                    resultPlayerId === pMemberId) ||
              ...
              if (player) {
                ...
                player.totalScore =
                  result.totalScore != null
                    ...
                    : player.totalScore;
                player.roundRank =
                  result.roundRank != null
                    ? Number(result.roundRank)
                    : player.roundRank;

                // 제출 결과 매칭
                const submission =
                  roundResultData.playerSubmissionResults?.[index];
                if (submission) {
                  player.distanceToTarget =
                    submission.distance != null
                      ? Number(submission.distance)
                      : player.distanceToTarget;
                  player.lastRoundScore =
                    submission.earnedScore != null
                      ? Number(submission.earnedScore)
                      : player.lastRoundScore;
          ...
          if (
            ...
            Array.isArray(roundResultData.playerGuesses)
          ) {
            const updatedPlayerGuesses = roundResultData.playerGuesses.map(
              (guess) => {
                // 플레이어 매칭: playerId와 memberId 모두 고려
                const player = this.gamePlayers.find((p) => {
                  const guessPlayerId =
                    guess.playerId != null ? String(guess.playerId) : null;
                  const guessMemberId =
                    guess.memberId != null ? String(guess.memberId) : null;
                  const pPlayerId =
                    p.playerId != null ? String(p.playerId) : null;
                  const pMemberId =
                    p.memberId != null ? String(p.memberId) : null;
                  ...
                  return (
                    (guessPlayerId &&
                      pPlayerId &&
                      guessPlayerId === pPlayerId) ||
                    (guessMemberId &&
                      pMemberId &&
                      guessMemberId === pMemberId) ||
                    (guessPlayerId &&
                      pMemberId &&
                      guessPlayerId === pMemberId) ||
        ...
        onPlayerSubmission: (submissionData) => {
          ...
          if (!playerId) {
            console.warn(
              "[Solo Game] 플레이어 제출 알림에 playerId가 없음:",
              submissionData,
            );
            return;
          }
          ...
          const player = this.gamePlayers.find((p) => {
            ...
            const submissionPlayerId =
              playerId != null ? String(playerId) : null;
            const submissionMemberId =
              memberId != null ? String(memberId) : null;

            // playerId로 먼저 매칭
            if (
              submissionPlayerId &&
              pPlayerId &&
              submissionPlayerId === pPlayerId
            ) {
              return true;
            }
          ...
          if (player) {
            ...
            console.warn(
              `[Solo Game] 플레이어를 찾을 수 없음: playerId=${playerId}, memberId=${memberId}`,
            );
          }
        },
      });
    ...
    processRoundData(message, isReIssue = false) {
      ...
      // 좌표 정보 추출 및 업데이트
      const targetLat =
        message.roundInfo?.targetLat ??
        message.targetLat ??
        message.location?.lat;
      const targetLng =
        message.roundInfo?.targetLng ??
        message.targetLng ??
    ...
    calculatePlayerAverageDistances() {
      this.gameStore.state.players.forEach((player) => {
        ...
        player.averageDistance =
          roundCount > 0 ? totalDistance / roundCount : 0;
      });
    },
    ...
    simulateRoundData() {
      ...
      const roundDataMessage = {
        ...
        roundInfo: {
          poiName:
            this.gameStore.state.locationInfo &&
            this.gameStore.state.locationInfo.name
              ? this.gameStore.state.locationInfo.name
              : "더미 지명",
        },
      };
